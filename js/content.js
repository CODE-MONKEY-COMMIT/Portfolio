                ...
                <div class="bottom">
                  ...
                  <div class="bottom__content">
                    ...
                    ${currentData.github ? `<a href="${currentData.github}" class="github" target="_blank">Github</a>` : ''}
                      <div href="" class="demo-live"><strong>페이지 보러가기 -></strong>
                      </div>
                    </div>
                  </div>
                </div>`;
...
contentList.addEventListener('click', (e) => {
  ...
  if (minBtn) {
    ...
    currentItem.remove();
    currentTask?.remove();
    const topItem = getHighestItem();
    if (topItem) {
      getRelatedTask(topItem.dataset.index)?.classList.add('active');
    }
  }
});
function minimizeWindow(currentTask, currentItem) {
  ...
  const topItem = getHighestItem();
  if (!topItem) return;
  liveIndexUpdate(topItem);
  getRelatedTask(topItem.dataset.index)?.classList.add('active');
}
function getHighestItem() {
  const remainingItems = contentList.querySelectorAll(
    '.content__item:not(.min)',
  );
  if (remainingItems.length === 0) return null;
  const liveIndexArray = Array.from(remainingItems).map((item) => {
    return Number(item.dataset.liveindex);
  });
  const highestLiveIndex = Math.max(...liveIndexArray);
  return contentList.querySelector(
    `.content__item[data-liveindex="${highestLiveIndex}"]`,
  );
}
...
taskList.addEventListener('click', (e) => {
  ...
  if (isMin) {
    ...
    activeTask(taskItem);
    liveIndexUpdate(relatedContent);
  }
});
function isExistEvent(isExist, currentIndex) {
  ...
  liveIndexUpdate(isExist);
}
function liveIndexUpdate(item) {
  highestZIndex++;
  item.dataset.liveindex = highestZIndex;
